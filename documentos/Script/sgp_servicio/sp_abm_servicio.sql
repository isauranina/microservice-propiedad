drop function if exists sp_abm_servicio;
CREATE OR REPLACE FUNCTION sp_abm_servicio(
    accion int
	,_num_sec bigint
...
begin
    case accion	
        -- -- select * from sp_abm_servicio(1,1,'descripcion','AC',1)
        when 1 then
        begin
            --Valida Datos
            
            --INSERTA DATOS
            insert into sgp_servicio(
				descripcion
				,estado
				) 
            values (
        ...
        when 3 then
        begin			
            perform num_sec from sgp_servicio where estado = 'BA' and num_sec = _num_sec::bigint limit 1;
            if found then
