drop function if exists sp_abm_estado_propiedad;
CREATE OR REPLACE FUNCTION sp_abm_estado_propiedad(
    accion int
	,_num_sec bigint
...
begin
    case accion	
        ...
        begin
            --Valida Datos
             --  select * from sp_abm_estado_propiedad(1,1,'descripcion','AC',1)
            --INSERTA DATOS
            insert into sgp_estado_propiedad(
				descripcion
				,estado
				) 
            values (
        ...
        --MODIFICAR
      	...
      	begin
            ...
            --ACTUALIZA DATOS
            UPDATE sgp_estado_propiedad SET 
        ...
        when 3 then
        begin			
            perform num_sec from sgp_estado_propiedad where estado = 'BA' and num_sec = _num_sec::bigint limit 1;
            if found then
                return QUERY select 'error', 'El Registro ya ha sido Eliminado', '0';	
                return;
            end if;
            update sgp_estado_propiedad
            set estado = 'BA' 
		where num_sec = _num_sec::bigint;			
            v_id:= _num_sec;
            ------Valida si se afectaron filas----------------
    ...
    -------adm_log---------
	--select * from sgp_estado_propiedad
