drop function if exists sp_abm_reglas_propiedad;
CREATE OR REPLACE FUNCTION sp_abm_reglas_propiedad(
    accion int
	,_num_sec bigint
...
begin
    case accion	
        ...
        begin
            ...
            --INSERTA DATOS
            insert into sgp_reglas_propiedad(
				nsec_propiedad
				,descripcion
				,fecha_registro
				,estado
        ...
        --MODIFICAR
      	...
      	begin
            ...
            --ACTUALIZA DATOS
            UPDATE sgp_reglas_propiedad SET 
        ...
        when 3 then
        begin			
            perform num_sec from sgp_reglas_propiedad where estado = 'BA' and num_sec = _num_sec::bigint limit 1;
            if found then
                return QUERY select 'error', 'El Registro ya ha sido Eliminado', '0';	
                return;
            end if;
            update sgp_reglas_propiedad
            set estado = 'BA' 
		where num_sec = _num_sec::bigint;			
            v_id:= _num_sec;
            ------Valida si se afectaron filas----------------
    ...
    -------adm_log---------
	--select * from sgp_reglas_propiedad
